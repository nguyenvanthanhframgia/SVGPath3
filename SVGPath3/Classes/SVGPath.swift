...
//  SVGPath
//
//  Created by nguyen.van.thanh on 11/14/16.
//  Copyright © 2016 nguyen.van.thanh. All rights reserved.
//
...
open class SVGPath {
    ...
    fileprivate func finishLastCommand () {
        for command in take(SVGPath.parseNumbers(numbers), _stride: stride, coords: coords, last: commands.last, callback: builder) {
...
private func take (_ numbers: [CGFloat], _stride: Int, coords: Coordinates, last: SVGCommand?, callback: SVGCommandBuilder) -> [SVGCommand] {
    var out: [SVGCommand] = []
    var lastCommand:SVGCommand? = last
    let count = (numbers.count / _stride) * _stride
    var nums:[CGFloat] = [0, 0, 0, 0, 0, 0];
    
    for i in stride(from: 0, to: count, by: _stride) {
        for j in 0 ..< _stride {
            nums[j] = numbers[i + j]
        }
